...
function onClick(){
    const usernameInput = document.querySelector("#usernameInput")      //1. 특정한 마크업 가져오기
    const value = usernameInput.value                                   //2. 가져온 마크업 value 속성 가져오기
    document.querySelector("#greeting").innerHTML = value               //3. 출력할 특정 DOM에 출력
}
...
function onClick2(){
    ...
    colorBox.style = "background-color:skyblue; color:white"
}


/*예제 4: CSS 클래스 토글하기 (다크 모드 예시)
'테마 변경' 버튼(id: 'themeBtn')을 클릭할 때마다 <body> 태그에 'dark-mode' 클래스를 추가하거나 제거(토글)하시오. */
function toggleFun(){                                   // 토글 -> on/off
    let body = document.querySelector("body")
    body.classList.toggle("dark-mode")                  // .classList.toggle("클래스명") : 클래스 목록에 특정 클래스명 대입/삭제
}
...
function removeFunc(){
    ...
    targetBox.remove()                                        // 마크업 삭제 .remove() 
}
...
function selectFunc(){
    let markUp = document.querySelector("select")
    let selectValue = markUp.value

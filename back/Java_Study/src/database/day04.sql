...
SELECT bpname from buy GROUP BY bpname; -- 제품명 기준으로 그룹하고 제품명 필드만 조회
/* 그룹당 단 하나의 대표값만 조회 */
...
SELECT count(*) FROM buy;       -- null 포함
/* [3] 그룹 절과 집계함수 */
select mid, SUM(bamount) 합계 from buy GROUP BY mid;/* 1) mid 기준으로 그룹하여 총 구매수량(bamount) */
SELECT mid , sum(bamount * bprice) 구매금액 from buy GROUP BY mid; /* 2) mid기준으로 총 구매금액 (수량*가격) */
SELECT COUNT(*), mid FROM buy GROUP BY mid; /* 3) mid(회원)별 판매 횟수 */

/* [4] 그룹절의 조건절 , having은 그룹후 조건  where은 그룹전 조건 */
SELECT * FROM buy WHERE bamount > 3;
SELECT mid , SUM(bamount) 구매수량 FROM buy 
    GROUP BY mid HAVING 구매수량 > 5;

/* [5] order by 정렬 , desc 내림차순 , asc 오름차순*/
SELECT * FROM member ORDER BY mdebut;
SELECT * FROM member ORDER BY mdebut DESC;
/* 다중정렬 => 첫번째 정렬 후 첫번째 정렬 필드 기준으로 중복이 존재한 경우 중복끼리 2차정렬 */
SELECT * FROM member ORDER BY maddr DESC , mdebut asc;

/* [6] limit : 결과 레코드 제한, 페이징처리 */
SELECT * FROM member LIMIT 2;
SELECT * FROM member LIMIT 0, 2; /* 0번 부터 2개 */
SELECT * FROM member LIMIT 5, 5; /* 5번 부터 5개 */

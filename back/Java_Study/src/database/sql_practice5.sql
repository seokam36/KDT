...
select 제품명, 제품가격
from pcategory inner join product on pcategory.카테고리번호_pk = product.카테고리번호_fk
where 카테고리명 = "노트북" ;
...
select 카테고리명, 제품명
from pcategory pc left join product p on pc.카테고리번호_pk = p.카테고리번호_fk;

-- [문제 7] 재고가 한 번도 등록되지 않은 제품의 제품명을 조회하세요.
select 제품명  from product p left join stock s on p.제품번호_pk = s.제품번호_fk
where s.재고수량 is null;


-- [문제 8] 각 카테고리별로 총 재고 수량의 합계를 카테고리명과 함께 조회하세요.
select 카테고리명 , sum(s.재고수량) `총 재고 수량` from pcategory p inner join product p2 on
p.카테고리번호_pk = p2.카테고리번호_fk inner join stock s on p2.제품번호_pk = s.제품번호_fk
group by 카테고리명;


-- [문제 9] 각 제품별로 총 재고 수량을 조회하고, 총 재고 수량이 많은 순서대로 정렬하여 제품명과 총재고수량을 표시하세요.
select 제품명 , sum(재고수량) `총 재고 수량` from pcategory p inner join product p2 on p.카테고리번호_pk = p2.카테고리번호_fk
inner join stock s on p2.제품번호_pk = s.제품번호_fk group by 제품명 order by `총 재고 수량` desc;